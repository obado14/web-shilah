...
export function HeroFace() {
  ...
  return (
    <section
      id="hero"
      className="relative h-screen min-h-[640px] h-[100dvh] w-full overflow-hidden bg-[#0a0a0a]"
    >
      {/* Full-VH Background Photograph */}
      <Image
        id="hero-img"
        ...
        priority
        sizes="100vw"
        className={`object-cover object-[center_16%] min-[768px]:object-[center_20%] min-[1024px]:object-[center_24%] transition-opacity duration-1000 ${
          isLoaded ? 'opacity-100' : 'opacity-0'
        }`}
      />

      {/* Strategic Gradient Overlays: Darkens only the text areas, keeps Shilah's face completely clear */}
      {/* 1. Left Gradient for Desktop/Tablet: Covers left 0-55% where text sits, leaves face (center 40-60%) clear */}
      <div className="pointer-events-none absolute inset-y-0 left-0 w-full min-[768px]:w-[58%] min-[1280px]:w-[50%] bg-gradient-to-r from-black/90 via-black/50 to-transparent" />

      {/* 2. Bottom Gradient for Mobile: Covers lower 55% where text sits, leaves face (top 15-40%) clear */}
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-[60%] min-[768px]:hidden bg-gradient-to-t from-black/95 via-black/60 to-transparent" />

      {/* 3. Subtle bottom border blend to the next section */}
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-black/60 to-transparent" />

      {/* Hero Content Overlay: Positioned bottom-left on desktop & bottom on mobile to never block his face */}
      <div className="absolute inset-0 z-10 flex flex-col justify-end pb-8 min-[768px]:pb-14 min-[1024px]:pb-16">
        <div className="mx-auto w-full max-w-[1280px] px-6 min-[830px]:px-10 min-[1140px]:px-16">
          <div className="max-w-full min-[768px]:max-w-[540px] min-[1140px]:max-w-[620px]">
            {/* Badges */}
            ...
            {/* Main Title */}
            <h1
              className="m-0 text-[36px] font-normal leading-[1.05] tracking-[-0.04em] text-white min-[600px]:text-[50px] min-[830px]:text-[64px] min-[1140px]:text-[74px]"
              style={{
            ...
            {/* Subtitle */}
            <p
              className="mt-3 text-[15px] leading-[1.55] text-[#e0e0e0] min-[600px]:text-[17px] min-[830px]:text-[18px]"
              style={{
